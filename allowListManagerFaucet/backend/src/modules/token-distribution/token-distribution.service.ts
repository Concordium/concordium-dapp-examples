...
export class TokenDistributionService implements OnModuleInit {
        ...
        for (const tokenConfig of this.tokenConfigs) {
            try {
                ...
                if (response.ok) {
                    ...
                    this.logger.log(
                        `Loaded metadata for ${tokenConfig.id}: icon=${tokenConfig.iconUrl}, description=${tokenConfig.description}`,
                    );
                }
            } catch (error) {
    ...
    async startTokenDistribution(dto: AddToAllowListDto): Promise<string> {
        ...
        const tokenConfig =
            this.tokenConfigs.find((c) => c.id === tokenId) ??
            bail(`Could not find configured token with ID ${tokenId}.`);
        ...
        if (currentBalance !== null) {
            throw new Error(
                `Account ${dto.userAccount} has previously held token ${tokenConfig.id} and is not eligible for distribution`,
            );
        }

        this.logger.log(
            `Starting token distribution process ${processId} for user ${dto.userAccount} on token ${tokenId} (allowList: ${tokenConfig.hasAllowList})`,
        );
    ...
    private async executeCombinedTokenOperation(
        ...
        try {
            ...
            this.logger.log(
                `Executing combined operation for ${userAccount} on token ${tokenIdStr} (allowList: ${hasAllowList})`,
            );
            ...
            this.logger.log(
                `Sending transaction: ${hasAllowList ? 'addAllowList + ' : ''}mint ${amount} + transfer to ${userAccount}`,
            );
            ...
            switch (result.summary.transactionType) {
                ...
                case TransactionKindString.Failed:
                    ...
                    const details = this.concordiumService.decodeCborError(
        ...
        try {
            ...
            if (tokenInfo.state.moduleState) {
                try {
                    ...
                    this.logger.log(
                        `Decoded state for token ${checkTokenId}: ${JSON.stringify(decodedState, null, 2)}`,
                    );
                    ...
                    if (typeof decodedState === 'object' && decodedState !== null && 'allowList' in decodedState) {
                        ...
                        this.logger.log(
                            `User ${userAccount} allow list status for token ${checkTokenId}: ${isOnAllowList}`,
                        );
                        return isOnAllowList;
                    }
                } catch (decodeError) {
