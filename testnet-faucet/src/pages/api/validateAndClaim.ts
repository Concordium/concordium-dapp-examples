...
export default async function handler(
  ...
  if (!XPostId || !receiver || !sender) {
    return res
      ...
      .json({
        error:
          "Missing parameters. Please provide XPostId, receiver and sender.",
      });
  }
  ...
  try {
    ...
    if (!response) {
      return res
        ...
        .json({
          error:
            "Unable to retrieve tweet details. The X Post ID might be invalid.",
        });
    }

    const tweetText = response.fullText;
    const isValid =
      tweetText.includes(receiver) && tweetText.includes(extraKeywordToVerify);
    ...
    if (!isValid) {
      return res
        ...
        .json({
          error:
            "X Post verification failed. Please make sure you do not modify the template text and that your address is present.",
        });
    }
    ...
    const accountTransaction = await createAccountTransaction(
      client,
      sender,
      receiver,
    );
    ...
    return res
      .status(200)
      .json({ transactionHash: transactionHash.toString() });
  } catch (e) {
    return res
      .status(500)
      .json({ error: `An unexpected error has occurred: ${e}` });
  }
}
