...
export default function Home() {
  ...
  const [isValidVerification, setIsValidVerification] = useState<
    boolean | undefined
  >();
  ...
  return (
    <div className={`min-h-screen ${IBMPlexMono.className}`}>
      ...
      <div className="h-24 w-full bg-[--teal] flex items-center justify-center sm:px-10">
        <p className="text-xl md:text-2xl text-center font-semibold text-white">
          ...
          >
            ...
            >
              {addressValidationError && (
                <p className="text-xs text-red-700 h-fit -mt-2 px-2">
                  {addressValidationError}
                </p>
              )}
            </SingleInputForm>
            ...
            <SingleInputForm
              ...
              inputDisabled={
                !address ||
                Boolean(addressValidationError) ||
                isValidVerification
              }
              submitButtonDisabled={
                !isValidTweetUrl || isValidVerification || isVerifyLoading
              }
            />
            <Step step={3} />
                    ...
                    <DisclosurePanel className="mt-2 space-y-2 text-black/60">
                      {item.response
                        ...
                        .map((responseText, reponseLineIndex) => (
                          <p key={`${questionIndex}-${reponseLineIndex}`}>
                            {responseText}
                          </p>
                        ))}
                    </DisclosurePanel>
      ...
      <div className="h-32 w-full bg-[--teal] flex items-center justify-center sm:px-10">
        <Image
          src={poweredByConcordium}
          alt="powered by"
          className="w-64 sm:w-72"
        />
      </div>
      <Dialog
        open={turnstileIsOpen}
        onClose={() => {}}
      ...
      {error && (
        <ErrorAlert errorText={error} onClose={() => setError(undefined)} />
      )}
    </div>
  );
}
