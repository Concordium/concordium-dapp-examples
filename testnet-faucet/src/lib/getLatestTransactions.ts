export default async function getLatestTransactions(): Promise<
  PartialTransaction[]
> {
  if (
    ...
    throw new Error(
      "NEXT_PUBLIC_EXPLORER_API_URL or NEXT_PUBLIC_SENDER_ADDRESS env vars undefined.",
    );
  }
  ...
  try {
    ...
    return transactionResponse.transactions.map(
      ({ blockTime, transactionHash }) => ({
        blockTime,
        transactionHash,
      }),
    );
