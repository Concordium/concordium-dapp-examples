...
import { CHAIN_ID, PING_INTERVAL_MS, WALLET_CONNECT_PROJECT_ID } from '../config';
...
import { trySendTransaction, sign, wrapPromise } from '../browserWalletHelpers';
import Footer from './Footer';
import { signMessage, trySend } from '../walletConnectHelpers';
        ...
        ResultAsync.fromPromise(
            detectConcordiumProvider().then((client) => {
                ...
                client.on('accountDisconnected', () => {
                    ...
                    void client.getMostRecentlySelectedAccount().then(setBrowserwalletConnectedAccount);
                });
    ...
    const handleSubmitSign = useCallback(() => {
        if (wallet === 'browserwallet') {
            ...
            trySendTransaction(
                ...
                wrapPromise((client, account) => sign(client, account, message)),
            );
            ...
            trySend(
                ...
                wrapPromise((client, session) => signMessage(client, session, CHAIN_ID, message)),
            ).then((r) =>
                r.match(
                        ...
                        {wallet === 'walletconnect2' && (
                            <>
                                ...
                                {walletconnect2Client?.match(
                                    ...
                                    (e) => <Alert variant="danger">Wallet Connect is not available: {e}.</Alert>,
                                )}
                            </>
                        )}
                    </>
